...
RUN apt-get update && apt-get install -y --no-install-recommends \
    build-essential cmake wget git ca-certificates postgresql-server-dev-all && rm -rf /var/lib/apt/lists/*
WORKDIR /web-static
...
RUN cmake -S . -B build -DCMAKE_BUILD_TYPE=Release -DBUILD_SCRIPTS=OFF -DBUILD_TESTS=OFF && cmake --build build --config Release

# Runtime stage
FROM debian:stable-slim
RUN apt-get update && apt-get install -y --no-install-recommends \
    libstdc++6 ca-certificates postgresql-server-dev-all && rm -rf /var/lib/apt/lists/*

WORKDIR /web-static
COPY --from=build /web-static/build/web/chess-web /web-static
COPY --from=build /web-static/web/www/ /web-static/www/
