...
let white = true;
let dragFrom = null;
let possMoves = new Set;
let STARTING_POSITION = startingPosition();
function dropHandler(ev) {
    var _a, _b;
    const toSquare = ev.target.closest('.square');
    if (!toSquare)
        return;
    ...
    if (!movingPiece)
        return;
    // Update model TODO: add complexer moves like rochade and en passant aka another fetch with the to input
    STARTING_POSITION[fr][ff] = null;
    STARTING_POSITION[tr][tf] = movingPiece;
    // remove piece span at from position
    (_b = dragFrom === null || dragFrom === void 0 ? void 0 : dragFrom.querySelector('.piece')) === null || _b === void 0 ? void 0 : _b.remove();
    // Remove existing piece at target (capture) if any // TODO: same as above: rochade, en passant
...
function dragOverHandler(ev) {
    var _a;
    const dataSquare = (_a = ev.target.closest('.square')) === null || _a === void 0 ? void 0 : _a.getAttribute('data-square');
    if (dataSquare && possMoves.has(dataSquare)) {
        ev.preventDefault();
...
function dragStartHandler(ev, span) {
    ...
    fetch('/drag-start', {
        ...
        possMoves.forEach(possMove => {
            const possSquare = document.querySelector(`div[data-square="${CSS.escape(possMove)}"]`);
            possSquare === null || possSquare === void 0 ? void 0 : possSquare.setAttribute('hint', 'true');
        });
    }).catch(console.error);
}
function dragEndHandler(span) {
    dragFrom = null;
    possMoves.clear();
    span.style.opacity = '';
    document.querySelectorAll('.square[hint="true"]').forEach(sq => sq.removeAttribute('hint'));
}
function addPieceDragHandlers(span) {
    span.addEventListener('dragstart', (ev) => { dragStartHandler(ev, span); });
    span.addEventListener('dragend', () => { dragEndHandler(span); });
}
function addSquareDragHandlers(square) {
    square.addEventListener('dragenter', (ev) => { dragEnterHandler(ev); });
    square.addEventListener('dragover', (ev) => { dragOverHandler(ev); });
    square.addEventListener('drop', (ev) => dropHandler(ev));
}
