...
void ChessBoard::initBoard() {
  board.push_back(
      new ChessTile(std::make_unique<ChessPiece>(Rook, true), 0, 0));
  board.push_back(
      new ChessTile(std::make_unique<ChessPiece>(Knight, true), 1, 0));
  board.push_back(
      new ChessTile(std::make_unique<ChessPiece>(Bishop, true), 2, 0));
  board.push_back(
      new ChessTile(std::make_unique<ChessPiece>(Queen, true), 3, 0));
  board.push_back(
      new ChessTile(std::make_unique<ChessPiece>(King, true), 4, 0));
  board.push_back(
      new ChessTile(std::make_unique<ChessPiece>(Bishop, true), 5, 0));
  board.push_back(
      new ChessTile(std::make_unique<ChessPiece>(Knight, true), 6, 0));
  board.push_back(
      new ChessTile(std::make_unique<ChessPiece>(Rook, true), 7, 0));
  for (int i = 0; i < boardWidth; i++) {
    board.push_back(
        new ChessTile(std::make_unique<ChessPiece>(Pawn, true), i, 1));
  }
  ...
  for (int i = 0; i < boardWidth; i++) {
    board.push_back(
        new ChessTile(std::make_unique<ChessPiece>(Pawn, false), i, 6));
  }
  board.push_back(
      new ChessTile(std::make_unique<ChessPiece>(Rook, false), 0, 7));
  board.push_back(
      new ChessTile(std::make_unique<ChessPiece>(Knight, false), 1, 7));
  board.push_back(
      new ChessTile(std::make_unique<ChessPiece>(Bishop, false), 2, 7));
  board.push_back(
      new ChessTile(std::make_unique<ChessPiece>(Queen, false), 3, 7));
  board.push_back(
      new ChessTile(std::make_unique<ChessPiece>(King, false), 4, 7));
  board.push_back(
      new ChessTile(std::make_unique<ChessPiece>(Bishop, false), 5, 7));
  board.push_back(
      new ChessTile(std::make_unique<ChessPiece>(Knight, false), 6, 7));
  board.push_back(
      new ChessTile(std::make_unique<ChessPiece>(Rook, false), 7, 7));
...
void ChessBoard::updateBoard() const {
  ...
  for (int y = 7; y >= 0; --y) {
    ...
    std::cout << "\x1b[0m"
              << std::endl; // Reset to default color after each line
  }
...
GameState ChessBoard::handleInput(const std::string &input) {
  ...
  if (input == "draw") {
    std::cout << color << "is offering a draw. Do you accept? Y or N."
              << std::endl;
    std::string answer;
    std::cin >> answer;
...
Pieces ChessBoard::getPossibleMovesKing(const ChessTile *fromTile,
                                        const bool castling) {
  Pieces possibleMoves;
  const std::vector directions = {
      std::pair(0, 1), std::pair(0, -1), std::pair(1, 1),  std::pair(1, -1),
      std::pair(1, 0), std::pair(-1, 0), std::pair(-1, 1), std::pair(-1, -1)};
  mergePossVec(possibleMoves,
               getPossibleMovesByDirectionSingle(fromTile, directions));
  if (castling && !isKingChecked(whitesTurn))
    mergePossVec(possibleMoves, getPossibleMovesCastling(fromTile));
  return possibleMoves;
}

Pieces ChessBoard::getPossibleMovesByDirection(
    const ChessTile *fromTile,
...
Pieces ChessBoard::getPossibleMovesByDirectionSingle(
    const ChessTile *fromTile,
    const std::vector<std::pair<int, int>> &directions) {
  Pieces possibleMoves;
  const int x = fromTile->getX();
...
void ChessBoard::filterPossibleMovesForChecks(const ChessTile *fromTile,
                                              Pieces &possibleMoves) {
  // std::cout << "possible Moves size before filter: " << possibleMoves.size()
  // << std::endl; check if the move would make the player checked which is an
  // illegal move
  std::erase_if(possibleMoves, [fromTile, this](ChessTile *toTile) {
    return isKingChecked(fromTile, toTile);
  });
  // std::cout << "possible moves size after filter: " << possibleMoves.size()
  // << std::endl;
}

bool ChessBoard::isInputMovePossible(const ChessTile *fromTile,
                                     const ChessTile *toTile) {
  ...
  if (itPossMove == possibleMoves.end()) {
    std::cout << "ChessBoard::isInputMovePossible: that is not a possible move"
              << std::endl;
    return false;
  }
  return true;
...
bool ChessBoard::isPossibleMove(const ChessTile *fromTile, ChessTile *toTile,
                                Pieces &possibleMoves) {
  ...
  if (toTile->piece != nullptr) { // there is a piece
    if (fromTile->piece->getType() == Pawn &&
        fromTile->getX() ==
            toTile->getX()) { // pawn only piece that can't capture piece
                              // directly in its path
      return false;
    }
    if (fromTile->piece->isWhite() !=
        toTile->piece->isWhite()) // piece is different color so add it
...
bool ChessBoard::isTileAttackedAndFree(const bool white,
                                       const Pieces &tilesToCheck) {
...
bool ChessBoard::isThreefoldRepetition() {
  return std::ranges::any_of(gameHistory, [this](const std::string &boardStr) {
    return std::ranges::count(gameHistory, boardStr) >= 3;
  });
...
bool ChessBoard::isKingCheckmate() {
  const Pieces pieceTiles =
      !whitesTurn ? getAllWhiteTiles() : getAllBlackTiles();
  for (const ChessTile *tile : pieceTiles) {
    Pieces possMoves;
    if (tile->piece->getType() == King) {
      possMoves = getPossibleMovesKing(tile, false);
    ...
    if (possMoves.size() > 0) {
      // std::cout << tile->piece->getShortName() << "at: " << tile->getX() << "
      // : " << tile->getY() << " still has possible Moves" << std::endl;
      return false;
    }
  }
...
bool ChessBoard::isDraw() {
  ...
  if (getAllPossibleMoves(!whitesTurn).size() <= 0 &&
      !isKingChecked(!whitesTurn))
  ...
  if (whitePieces.size() == 2 && blackPieces.size() == 2) {
    ...
    if (itBlackBishop == blackPieces.end())
      return false;
    if ((*itWhiteBishop)->piece->isWhite() ==
        (*itBlackBishop)->piece->isWhite())
      return true;
    return false;
  }
...
// of them
Pieces ChessBoard::getAllPossibleMovesPiece(const bool white,
                                            const ChessPieceType piece) {
  Pieces allPossibleMoves;
  const Pieces colorPieceTiles =
      white ? getAllWhiteTiles() : getAllBlackTiles();
...
Pieces ChessBoard::getAllPiecesFor(const bool white,
                                   const ChessPieceType piece) const {
  Pieces pieces;
  const Pieces colorPieceTiles =
      white ? getAllWhiteTiles() : getAllBlackTiles();
...
GameState ChessBoard::handleMoveInput(std::string input) {
  ...
  if (!moveTilePair) {
    std::cout << "ChessBoard::handleMoveInput: invalid input: " << input
              << std::endl;
  }
...
void ChessBoard::movePawn(const ChessTile *fromTile, const ChessTile *toTile) {
  ...
  if (fromTile->getX() != toTile->getX() && toTile->piece == nullptr) {
    ...
    ChessTile *capturedPiece =
        getTileAt(toTile->getX(), toTile->getY() + whiteMove);
...
void ChessBoard::pawnWon(ChessTile *pawnTile, const char pawnToPiece) const {
  updateBoard();
  std::cout << "Your pawn reached the end what should it become? (Q, R, B, N)"
            << std::endl;
  char newPawnTyp;
  if (pawnToPiece == '0')
    std::cin >> newPawnTyp;
  ...
  while (!(newPawnTyp == 'Q' || newPawnTyp == 'R' || newPawnTyp == 'B' ||
           newPawnTyp == 'N')) {
...
GameState ChessBoard::afterMoveChecks(ChessTile *toTile,
                                      const char pawnToPiece) {
  enPassantPossibleLastMove = false;
  doublePawnMoveAt = -1;
  if (toTile->piece->getType() == Pawn &&
      (toTile->getY() == 0 || toTile->getY() == 7)) {
...
std::optional<std::pair<ChessTile *, ChessTile *>>
ChessBoard::getMoveTilesFromInput(const std::string &input) const {
  if (input.length() != 5) {
    std::cout
        << "ChessBoard::getMoveTilesFromInput: Wrong input length, should be 5."
        << std::endl;
    return std::nullopt;
  }
  ...
  if (fromTile == nullptr || toTile == nullptr) {
    std::cout << "ChessBoard::getMoveTilesFromInput: there is no tile like that"
              << std::endl;
    return std::nullopt;
  }
